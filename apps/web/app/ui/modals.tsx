...
export function VideoReview({ videoId, onClose, onSave }: { videoId: string; onClose: () => void; onSave: () => void }) {
  ...
  const [windowKeyframes, setWindowKeyframes] = useState<TrackedKeyframe[]>([]);
  ...
  const serverTrackingAvailable = analysis?.engine === "AquaVision" && (!!analysis.keyframeSegments?.length || !!analysis.keyframes?.length);
  useEffect(() => {
    if (!serverTrackingAvailable) {
      setWindowKeyframes([]);
      return;
    }
    const from = Math.max(0, currentTime - 5);
    const to = currentTime + 5;
    const timeout = window.setTimeout(() => {
      void apiRequest<{ keyframes: TrackedKeyframe[] }>(`/api/v1/videos/${videoId}/keyframes?from=${from}&to=${to}`)
        .then((result) => setWindowKeyframes(result.keyframes))
        .catch(() => setWindowKeyframes([]));
    }, 100);
    return () => window.clearTimeout(timeout);
  }, [videoId, currentTime, serverTrackingAvailable]);
  ...
  const phaseLabel = activeEvent?.label ?? (inGap && focus ? `Track técnico #${focus.id} sem rastreio (${clock(inGap.from)}–${clock(inGap.to)})` : analysis && !tracksAthletes ? "Sem tracks técnicos" : "");
  ...
  const engineNote = analysis
    ? tracksAthletes
      ? `${people.length} ${people.length === 1 ? "track técnico" : "tracks técnicos"}${analysis.metadata.calibrationSnapshot ? ` · ${analysis.metadata.calibrationSnapshot.cameraId} · calibração v${analysis.metadata.calibrationSnapshot.version} · cobertura ${Math.round(analysis.metadata.calibrationSnapshot.coverage * 100)}% · métricas indisponíveis: ${analysis.metadata.metricAvailability?.avgSpeed.reason ?? "extrator não validado"}` : " · sem snapshot de calibração"}${coverageEndsAt !== null ? ` · pose sincronizada até ${coverageEndsAt.toFixed(0)} s` : ""}`
      : "Este motor mede apenas o movimento global da cena: não identifica pessoas, braçadas nem velocidade."
    : "";
  return <ModalShell title={`${record.athlete ?? fallback.athlete} · ${record.event ?? record.title ?? fallback.event}`} subtitle={`Análise sincronizada · ${analysis?.engine ?? "AquaMotion"} ${analysis?.engineVersion ?? ""}`} onClose={onClose} wide>
    ...
    <div className="review-layout real-review"><div><div className="review-player"><div className="real-video-stage"><video ref={player} src={mediaUrl(record.url)} poster={mediaUrl(record.thumbnailUrl)} playsInline preload="metadata" onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} /><button type="button" className={`pose-toggle ${poseEnabled ? "active" : ""}`} onClick={() => setPoseEnabled((value) => !value)} aria-pressed={poseEnabled} title={serverTrackingAvailable ? "Rastreamento AquaVision do servidor, carregado pela janela de reprodução" : "Rastreamento de pose em tempo real no navegador"}><Sparkles size={15} />IA{serverTrackingAvailable ? " · servidor" : ""}</button>{poseEnabled && <PoseTrackingLayer videoRef={player} active serverKeyframes={windowKeyframes} serverTrackingAvailable={serverTrackingAvailable} serverPersonIds={personIds} selectedPersonId={selectedPerson} coverageEndsAt={coverageEndsAt} />}<button className="video-play-control" onClick={() => void toggle()}>{playing ? <Pause size={23} fill="currentColor" /> : <Play size={23} fill="currentColor" />}</button><span className="player-time">{clock(currentTime)} / {clock(duration)}</span>{phaseLabel ? <span className="active-phase">{phaseLabel}</span> : null}</div><div className="timeline live-timeline" onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - rect.left) / rect.width) * duration); }}><i style={{ width: `${duration ? currentTime / duration * 100 : 0}%` }} />{focus?.gaps?.map((gap) => <s key={`${gap.from}-${gap.to}`} className="timeline-gap" title={`Sem rastreio ${clock(gap.from)}–${clock(gap.to)}`} style={{ left: `${duration ? gap.from / duration * 100 : 0}%`, width: `${duration ? (gap.to - gap.from) / duration * 100 : 0}%` }} />)}{allEvents.map((event) => <em key={event.id} title={`${event.label} · ${clock(event.time)}`} style={{ left: `${duration ? event.time / duration * 100 : 0}%`, background: typeof event.personId === "number" ? trackColor(event.personId, personIds) : zoneDistribution[4].color }} />)}</div></div>
      ...
      <TrackAssignmentPanel videoId={videoId} trackIds={people.map((person) => String(person.id))} />
      ...
      <aside className="review-sidebar">
        {tracksAthletes ? <div className="tracked-athletes"><span className="eyebrow">TRACKS TÉCNICOS · {people.length}</span>{people.slice(0, 6).map((person) => {
          ...
          return <button type="button" className={`tracked-athlete-row ${selectedPerson === person.id ? "active" : ""}`} key={person.id} onClick={() => setSelectedPerson(selectedPerson === person.id ? null : person.id)}><span className="tracked-athlete-dot" style={{ background: trackColor(person.id, personIds) }} /><div><b>Track técnico #{person.id}</b><small>{clock(person.firstSeen)}–{clock(person.lastSeen)} · {cycles ? "ciclos não validados" : typeof person.strokes === "number" ? `${person.strokes} braçadas` : "braçadas indisponíveis"} · {rate.value} · {avg.value}{avg.note ? ` (${avg.note})` : ""}</small></div><em>{person.gaps?.length ? `${person.gaps.length} lacuna${person.gaps.length > 1 ? "s" : ""}` : "contínuo"}</em></button>;
        })}</div> : null}
     ...
     {saveError && <p className="modal-error review-save-error" role="alert">{saveError}</p>}<footer className="modal-footer"><div className="analysis-summary"><Sparkles size={15} /><span>{analysis ? tracksAthletes && focus ? <><b>Track técnico #{focus.id}: métricas esportivas indisponíveis</b> · motor {analysis.engine}</> : <><b>{analysis.metrics.detectedCycles ?? 0} picos de movimento</b> · sem identificação de pessoas · motor {analysis.engine}</> : "Sem análise"}</span></div><button className="secondary-button" onClick={onClose}>Fechar</button><button className="primary-button" onClick={() => void save()}><Check size={16} />Salvar revisão</button></footer>
  </ModalShell>;
}
